...
public class Asm {
    ...
    private static Pattern classPattern = Pattern.compile("\\.(class|interface) (.*?)?([\\w/]+)$");
    ...
    private static Pattern superPattern = Pattern.compile("\\.super ([\\w/]+)$");
    ...
    private static Pattern implementsPattern = Pattern.compile("\\.implements ([\\w/]+)$");
    ...
    private static Pattern fieldPattern  = 
        Pattern.compile(".field +(.*?)?([$\\w]+) +([\\[\\w/;]+) *(= *(.*))?");
    private void parseField() {
        String name       = group(2);
        String desc       = group(3);
        String valueStr   = group(4);
    ...
    private static Pattern methodPattern = Pattern.compile(".method(.*)? ([<>\\w]+) *([(][^\\s]+)");
    ...
    private static Pattern throwsPattern = Pattern.compile("^.throws +([\\w+/])");
    ...
    static Pattern catchPattern      = Pattern.compile(".catch +([\\w/]+) +from +([\\w]+) +to +([\\w]+) +using +([\\w]+)");
    ...
    static final Pattern methodSpecPattern = Pattern.compile("([\\w/]+)[/.]([<>\\w]+) *([(].*)");
    ...
    private void parseInstructions() {
        ...
        switch (visitTypes[opcode]) {
            ...
            case METHOD: {
                ...
                if (operand == null || !match(operand, methodSpecPattern)) {
