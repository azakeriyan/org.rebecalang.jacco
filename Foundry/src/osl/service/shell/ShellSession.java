...
/**
   ...
   This class actually handles the sessions opened by the shell
   service. 
...
public class ShellSession implements Runnable {
  ...
  */
  ...
  /**
     A hashtable holding all the objects instantiated by this
     session. 
  */
  Hashtable<Object, Object> symbols;
  
  /**
     A reference to the socket we use to interact with the shell
     client. 
  */
  Socket client;

  /**
     An integer which is used to generate the unique part of each
     shell variable name for this shell session.
  */
  int nextShellName = 0;

  /**
     The input stream tokenizer which we use to read new tokens for
     this session.
  */
//   StreamTokenizer inReader = null;
  ...
  */
  ...
  /**
     The output stream to which we should write replies for this
     session.
  */
  PrintWriter outWrite;

  /**
     The name of the <em>ShellActorImpl</em> associated with this
     shell session.
  */
  ActorName ourActor;

  /**
     A direct reference to the <em>ShellActorImpl</em> associated with
     this shell session.  This value is set by our managing parent
     after the <em>ShellActorImpl</em> is created and invokes the
     shell service with a reference to itself.
  */
  ShellActorImpl ourActorRef;

  /**
     This queue holds incoming messages we have received through our
     local <em>ShellActorImpl</em>.  We deliver these messages on
     request to the client.
  */
  Queue<ActorMsgRequest> msgQueue;

  /**
     These constants indicate what type of stream should be assigned
     within the EstablishStreamConnect inner class.
  */
  ...
  /**
     ...
     @param <b>C</b> The <em>Socket</em> we should use to interact
     with the client for this shell.
     @param <b>I</b> The <em>int</em> ID associated with this
     session.  This value is unique local to this shell service
     instance.
  */
  ...
  */
  ...
  /**
     This is where a shell session initially starts.  We first create
     a set of streams for client interaction.  A
     <em>ShellActorImpl</em> is instantiated locally in order to serve 
     as a surrogate for the shell.  When this actor is created it
     obtains a direct reference to this instance, and calls
     <em>shellMainLoop</em> with its local thread.  This gives the
     shell session the proper permissions required for calling the
     "actor" functions in the local actor manager.  When
     <em>shellMainLoop</em> exits, the shell session is terminated.
  */
  public void run() {
    try {
      // Log our output
//      Log.logThread("ShellService", Thread.currentThread());
	...
	ActorCreateRequest shellActorCreate = 
	  new ActorCreateRequest(null, Actor.getClassRef(), ShellActorImpl.classRef, args, null);
	...
	outWrite.println("Unable to create shell actor, closing connection");
	outWrite.flush();
	client.close();
...
//      Log.logExceptionTrace(e);
  ...
  /**
     This static table defines the legal characters that make up a
     token.  Strings are a special case which may contain any
     arbitrary sequence of characters delineated by quotes.
  */
  ...
  /**
     Parse the next token from the input stream.  This method returns
     a command token representing the next token read from the
     stream.  We block if there are not enough characters in the
     stream to make a complete token.  Returns null if the end of the
...
//      Log.println("<ShellSession.getNextToken>: received exception " + e);
      return null;
    }
  ...
  /**
     This is the main processing loop for this shell session.  The
     loop is actually executed by the thread allocated to a
     <em>ShellActorImpl</em> instance.  This is done so that
     permissions are set correctly when the user requests to send
     messages or create actors.
  */
  void shellMainLoop() {
...
//      Log.println("<ShellSession.shellMainLoop>: Starting main shell processing loop...");
      ...
      parent.ourScheduler.scheduleThread(new Thread(new EstablishStreamConnect(ourActorRef, newStreamListen, SET_STDIN), "stdin"));

      newStreamListen = new ServerSocket(0);
      portSpecs       = portSpecs + newStreamListen.getLocalPort() + " ";
      parent.ourScheduler.scheduleThread(new Thread(new EstablishStreamConnect(ourActorRef, newStreamListen, SET_STDOUT), "stdout"));

      newStreamListen = new ServerSocket(0);
      portSpecs       = portSpecs + newStreamListen.getLocalPort() + " SV_END";
      parent.ourScheduler.scheduleThread(new Thread(new EstablishStreamConnect(ourActorRef, newStreamListen, SET_STDERR), "stderr"));
...
//	    Log.println("<ShellSession.shellMainLoop>: Shell Stream Closed, terminating...");
	  ...
	  case Shell.CMD_INVOKE:
	    // Invoke a method on an object stored in a shell variable
	    cmdInvoke(tokens);
	    break;

	  case Shell.CMD_CREATE:
	    // Create a new actor and save the name in a new shell variable
	    cmdCreate(tokens);
	    break;
	  ...
	  case Shell.CMD_MSGDELETE:
	    // Delete the first message in the mail queue.  Returns an
	  ...
	  case Shell.CMD_SERVICE:
	    // Invoke a local node service.  Returns the result of the 
	  ...
	  default:
	    ...
	    outWrite.println("SV_ERROR unknown command \"" + cmd + "\" SV_END");
	    outWrite.flush();
	  }
	}
      }
	...
	// Since this is just cleanup code, we ignore any exception here.
      }
//      Log.println("<ShellSession.shellMainLoop>: Shell Stream Closed: returned exception " + e);
//      Log.logExceptionTrace(e);
    }
  }

  /**
     Receive a new message from our associated
     <em>ShellActorImpl</em>.  At the moment we just put the message
     in our local message queue.
  */
  ...
  /**
     Handle the "instobj" shell command.  Instantiates a new object of 
     an arbitrary class using the given set of arguments.  Saves the
     new object in a fresh shell variable and outputs the name of the
     variable to the client.
  */
  void cmdInstObj(Queue<CmdToken> tokens) {
    // Arg 1 - Interpreted as the fully qualified class name of the object to create
    // Arg 2 -> Arg N (optional) - Arguments to pass to the constructor
    if (tokens.numElements() < 1) {
      outWrite.println("SV_ERROR usage: instobj className [arg1 ... argN] SV_END");
      outWrite.flush();
      return;
    }
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception while resolving class: " + e + " SV_END");
      outWrite.flush();
      return;
    }
	...
	outWrite.println("SV_ERROR exception parsing argument: " + e + " SV_END");
	outWrite.flush();
	return;
      }
      i++;
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception finding constructor: " + e + " SV_END");
      outWrite.flush();
...
//      Log.logExceptionTrace(e);
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception creating object: " + e + " SV_END");
      outWrite.flush();
      return;
    }
  ...
  /**
     Handle the "invoke" shell command.  Invokes an arbitrary method
     on an object stored in a shell variable.  If the method has no
     return value then "null" is printed on the output stream.
     Otherwise a new shell variable is created to hold the return
     value and the name of the new variable is printed on the output
     stream.
  */
  void cmdInvoke(Queue<CmdToken> tokens) {
    ...
    if (tokens.numElements() < 2) {
      outWrite.println("SV_ERROR usage: invoke shellVar method [arg1 ... argN] SV_END");
      outWrite.flush();
      return;
    }
    ...
    try {
      targObj = parent.parseArgument( ((CmdToken) tokens.dequeue()).text, symbols );
    } catch (Exception e) {
      outWrite.println("SV_ERROR exception parsing shell var: " + e + " SV_END");
      outWrite.flush();
      return;
    }
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception finding method: " + e + " SV_END");
      outWrite.flush();
...
//      Log.logExceptionTrace(e);
	...
	outWrite.flush();
      ...
      outWrite.println("SV_ERROR exception invoking method: " + e + " SV_END");
      outWrite.flush();
...
//      Log.logExceptionTrace(e);
  ...
  /**
     Handle the "print" shell command.  Equivalent to invoking
     "toString" on an object stored in shell variable.  Prints the
  ...
  */
  ...
  /**
     Handle the "create" shell command.  Creates a new actor and
     stores the name of the actor in a new shell variable.  The name
     of this variable is printed on the output stream.
   */
  void cmdCreate(Queue<CmdToken> tokens) {
    ...
    if (tokens.numElements() < 3) {
      outWrite.println("SV_ERROR usage: create node actorImplClass actorClass [arg1 ... argN]  SV_END");
      outWrite.flush();
      return;
    }
    ...
    try {
      where = (ActorManagerName) parent.parseArgument(((CmdToken) tokens.dequeue()).text , symbols);
    } catch (Exception e) {
      outWrite.println("SV_ERROR exception parsing node argument: " + e + " SV_END");
      outWrite.flush();
      return;
    }
	...
	outWrite.println("SV_ERROR error resolving implementation or behavior type: " + e + " SV_END");
	outWrite.flush();
...
//	Log.logExceptionTrace(e);
	...
	outWrite.println("SV_ERROR exception parsing argument " + nextArg + " : " + e + " SV_END");
	outWrite.flush();
	return;
      }
      i++;
	...
	f = ((RemoteCodeException) e).detail;
      outWrite.println("SV_ERROR exception creating new actor: " + f + " SV_END");
      outWrite.flush();
...
//      Log.logExceptionTrace(e);
  ...
  /**
     Handle the "send" shell command.  Sends a message to an actor
     who's name is contained in a shell variable.  A message consists
     of a method name and a set of arguments to pass to the invoked
     method.
  */
  void cmdSend(Queue<CmdToken> tokens) {
    ...
    //                    the target actor.
    if (tokens.numElements() < 2) {
      outWrite.println("SV_ERROR usage: send shellVar meth [arg1 ... argN] SV_END");
      outWrite.flush();
      return;
    }
    ...
    try {
      ...
      outWrite.println("SV_ERROR target argument " + actName + " must be an ActorName: " + e + " SV_END");
      outWrite.flush();

//      Log.logExceptionTrace(e);
      return;
    } catch (Exception e) {
      outWrite.println("SV_ERROR error parsing symbol " + actName + " : " + e + " SV_END");
      outWrite.flush();
...
//	Log.logExceptionTrace(e);
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception sending message: " + e + " SV_END");
      outWrite.flush();
...
//      Log.logExceptionTrace(e);
  ...
  /**
     Handle the "msgqueuesize" command.  Returns an integer giving the 
     current size of the incoming mail queue.
  */
  ...
  /**
     Handle the "msggetmethod" command.  Returns the string naming the 
     method of the first message in the mail queue.  Returns an error
     if the mail queue is empty.
  */
  void cmdMsgGetMethod(Queue<CmdToken> tokens) {
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception extracting first message: " + e + " SV_END");
      outWrite.flush();
    }
  ...
  /**
     Handle the "msggetargs" command.  Creates a new shell variable to 
     hold each argument passed in the first message in the mail
  ...
  */
  ...
  /**
     Handle the "msggetsender" command.  Creates a new shell variable
     which holds the actor name of the sender of the first message in
     the mail queue.  Returns an error if the mail queue is emtpy.
  */
  ...
  /**
     Handle the "msgisrpc" command.  Outputs "true" if the first
     message in the mail queue represents an RPC request, and "false"
     otherwise.  Returns an error if the mail queue is empty.
  */
  ...
  /**
     Handle the "msggettags" command.  Builds a set of (key, value)
     shell variable pairs to hold the contents of the tag table.  That 
     is, an even number of new shell variables is always created.
     Returns an error if the mail queue is empty.
  */
  ...
  /**
     Handle the "msgdelete" command.  Deletes the first message in the
     mail queue.  Returns an error if the mail queue is empty.
  */
  ...
  /**
     Handle the "makeglobal" command.  Stores the value of the shell
     variable argument into the global variable table for the shell
     service.  Returns the name of the new global variable or an error 
     if the global var does not exist.
  */
  void cmdMakeGlobal(Queue<CmdToken> tokens) {
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception parsing argument " + theArg + ": " + e + " SV_END");
      outWrite.flush();
      return;
    }
  ...
  /**
     Handle the "service" command.  Invokes a service and stores the
     result in a new shell variable.  Returns the name of the new
     global variable or "null" if the invocation had no result.
  */
  void cmdService(Queue<CmdToken> tokens) {
    ...
    if (tokens.numElements() < 2) {
      outWrite.println("SV_ERROR usage: service service-name meth [arg1 ... argN] SV_END");
      outWrite.flush();
      return;
    }
    ...
    try {
      servName = (ServiceName) parent.parseArgument( ((CmdToken) tokens.dequeue()).text, symbols );
    } catch (Exception e) {
      outWrite.println("SV_ERROR exception parsing service name: " + e + " SV_END");
      outWrite.flush();
      return;
    }
...
//	Log.logExceptionTrace(e);
    ...
    try {
      ...
      outWrite.println("SV_ERROR exception invoking service: " + e + " SV_END");
      outWrite.flush();
      return;
    }
  ...
  /**
     Helper method for checking if the mail queue is empty.  This
     method is used in several places to verify the mail queue is
     non-empty before performing a particular action.  Returns "true"
     if the mail queue is emtpy, and false otherwise.
  */
  boolean queueEmpty() {
    if (msgQueue.empty()) {
  ...
  /**
     This is just a container class for holding the contents and type
     of parsed tokens.
  */
  class CmdToken {
    /**
    ...
    */
    ...
    /**
       True if the token corresponds to a quoted string, otherwise
       this token represents a "word".
    */
    boolean isString;
