		...
		while (true) {
			if (!open) {
				synchronized (barrierSemaphore) {
					synchronized (privateSchedule) {
						if (barrierSemaphore.get() == currentWorkerPoolSize && privateSchedule.empty()) {
			...
			try {
				...
				if (counter == 20)	{
					...
					// prevent starvation due to multiple non-cooperative actors by expanding pool
					if (!progress && !privateSchedule.empty())	{
						WorkerThread workerThread = new WorkerThread();
						workerPool.add(workerThread);
						workerThread.start();
