...
/**
   A simple test of the basic actor manager implementation using the
   HelloWorld example.
...
public class TestManager implements Runnable {
  ...
  public static void main(String[] argv) {
    try {
      ...
      if (argv.length < 3)
	throw new RuntimeException("Args: hostname classType method [methodArgs]");
  ...
  public void run() {
    try {
      ...
      ((UDPTransportLayer) theLayer).transportInitialize(theScheduler, host);
      ...
      ActorCreateRequest createReq = new ActorCreateRequest(null, 
							    Class.forName(classType),
							    Class.forName("osl.manager.basic.BasicActorImpl"),
							    new Object[0], null);
      ActorName newAct = theManager.managerCreate(createReq, null);

      // Good, now submit a message request and that should be it.
      ActorMsgRequest newMsg = new ActorMsgRequest(null, newAct, method, args, false);
