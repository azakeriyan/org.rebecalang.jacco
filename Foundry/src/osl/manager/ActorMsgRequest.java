...
/**
  ...
  This class packages a request to send a message.  Instances of this
  class are passed from actors to their managers in order to request a 
  message send. <p>
...
public class ActorMsgRequest extends ActorRequest {
  ...
  */
  ...
  /**
     The arguments to pass to the invoked method in the receiver.  If
     no arguments are required then THIS SHOULD BE AN ARRAY OF LENGTH
     0, rather than null.
  */
  public Object[]  methodArgs;

  /**
     Set to <b>true</b> if this message has blocked the sender waiting
     for a reply, <b>false</b> otherwise.
  */
  ...
  */
  ...
  /**
     The usual constructor that is used to build messages.  This
     construct accepts values for each of the message fields, but sets
     the <em>RPCRequest</em> field to <b>false</b>.
  */
  ...
  */
  public String toString() {
    return "<ActorMsgRequest: " + super.toString() + " sender=" + sender + " receiver=" +
      receiver + " method=" + method + " numArgs=" + methodArgs.length +
      " rpc?=" + RPCRequest + ">";
  }

  /**
     Provide a "safe" clone of a message.  A safe message clone is a
     regular clone of the original message with the exception that a
     deep copy is performed on the <em>methodArgs</em> field.  This
     allows cloning to be used for handing a message off during local
     message passing.  That is, the invariant is that the
     <em>methodArgs</em> structure must be fresh to avoid hidden
     channels between user actors.
     ...
     in the <tt>methodArgs</tt> field.  This field is serialized in
     order to create a deep copy.  We use a deep copy to prevent the
     creation of hidden channels between actors sharing a reference to 
     a common object.

  */
  public Object clone() throws ActorRequestCloneException {
    // PRAGMA [debug,osl.manager.ActorMsgRequest] Log.println("<ActorMsgRequest.clone> Here 0...");
    ...
    try {
      // PRAGMA [debug,osl.manager.ActorMsgRequest] Log.println("<ActorMsgRequest.clone> Here 1...");
      // This dirty trick knocks several ms off the standard cloning time
      if (osl.foundry.FoundryStart.useNative) {
	// PRAGMA [debug,osl.manager.ActorMsgRequest] Log.println("<ActorMsgRequest.clone> Here 2...");
	newCopy.methodArgs = new Object[methodArgs.length];
 	for (int i=0; i<methodArgs.length; i++)
 	  if ( (methodArgs[i] != null) &&
 	       (DeepCopy.isMutable(methodArgs[i].getClass().toString())) ) 
 	    newCopy.methodArgs[i] = DeepCopy.deepCopy((Serializable) methodArgs[i]);
          else
            newCopy.methodArgs[i] = methodArgs[i];
      } else
	// PRAGMA [debug,osl.manager.ActorMsgRequest] {
	// PRAGMA [debug,osl.manager.ActorMsgRequest] Log.println("<ActorMsgRequest.clone> About to clone methodArgs...");
	// PRAGMA [assert] Assert.assert(methodArgs != null, "Method args should not be null here");
	...
	// PRAGMA [debug,osl.manager.ActorMsgRequest] Log.println("<ActorMsgRequest.clone> Done cloning methodArgs...");
	// PRAGMA [debug,osl.manager.ActorMsgRequest] }
    } catch (IOException e) {
      // Serialization error for methodArgs
      throw new ActorRequestCloneException("error serializing methodArgs in ActorMsgRequest", e);
    } catch (Exception e) {
